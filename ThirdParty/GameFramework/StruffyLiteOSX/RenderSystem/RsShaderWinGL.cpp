...
void RsShaderWinGL::FixPointers( BtU8 *pFileData, BaArchive *pArchive )
{
	...
	for( BtU32 shaderIndex=0; shaderIndex<m_pFileData->m_count; shaderIndex = shaderIndex + 2 )
	{
        ...
        BtU32 versionStringSize = sizeof("#version 123\n");
        BtU32 vertexShaderSize = vertexShaderData.m_size + versionStringSize;
        BtU32 pixelShaderSize  = pixelShaderData.m_size + versionStringSize;
        
        // Create the string to prepend on the front of our shaders
        BtFloat glLanguageVersion;
        sscanf((char *)glGetString(GL_SHADING_LANGUAGE_VERSION), "%f", &glLanguageVersion);
        int versionNumber = (int)( 100.0f * glLanguageVersion );
        BtChar prependVersionString[32];
        sprintf(prependVersionString, "#version %d\n", versionNumber );

        BtChar newText[8192];
        strcpy( newText, prependVersionString );
        strcat( newText, vertexText );
        vertexText = newText;
        
        glShaderSource( vertexShader, 1, &vertexText, (const GLint*)&vertexShaderSize );
        ...
        strcpy( newText, prependVersionString );
        strcat( newText, fragmentText );
        fragmentText = newText;

        glShaderSource( fragmentShader, 1, &fragmentText, (const GLint*)&pixelShaderSize );
