...
void RsMaterialWinGL::SetZTest(BtBool enabled)
{
	if(enabled)
	{
...
void RsMaterialWinGL::SetTechniqueName( const BtChar* pName )
{
	BtStrCopy( m_pFileData->m_techniqueName, 64, pName );
}
...
void RsMaterialWinGL::Render( RsPrimitiveType primitiveType, RsVertex3 *pVertex, BtU32 vertexCount, BtU32 sortOrder, BtBool isVertexCopied )
{
	// Cache the impl
	...
	{
		pPrimitive->m_primitiveType = GL_TRIANGLE_FAN;
	}
	...
	pMaterialRenderable->m_pVertex   = pVertex;
...
void RsMaterialWinGL::Render( RsPrimitiveType primitiveType, RsVertex2 *pVertex, BtU32 vertexCount, BtU32 sortOrder )
{
	...
	{
		primitive->m_primitiveType = GL_LINE_STRIP;
	}
...
void RsMaterialWinGL::Render( RsMaterialRenderable *pRenderable )
{
	...
	if( strstr( this->m_pTitle, "white3lit" ) )
	{
		int a=0;
		a++;
	}
